...
مفتاح Staging العام لا يُختار اعتمادًا على بادئته فقط. قد تحتوي حزمة قديمة أكثر من مفتاح عام، وقد لا يقبل مشروع Staging أحدها. يجب قبل البناء إرسال طلب قراءة محمي إلى `company_settings` باستخدام المفتاح المرشح في رأسي `apikey` و`Authorization` واشتراط الحالة `200`، ثم تكرار الاختبار باستخراج المفتاح من ملف JavaScript الناتج نفسه قبل `rsync`. أي `401` يوقف النشر، وإذا ظهر بعد النشر تُعاد نسخة الواجهة السابقة فورًا قبل التحقيق. لا تستخدم أمرًا يعيد كل مفاتيح المشروع لأن ناتجه قد يشمل مفاتيح إدارية غير مطلوبة.
