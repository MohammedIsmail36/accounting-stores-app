...
بُنيت محليًا بعد ذلك دفعة «اعتماد المعالجة» فقط. لا يظهر الإجراء إلا للمدير عندما تكون الحالة `ready_for_review`، ويستدعي `approve_inventory_reconciliation_repair` مع `expected_version` و`request_id` ثابت للمحاولة. إذا كان المدير هو نفسه مُعدّ المسودة، تفرض الواجهة سببًا صريحًا لعدم فصل المهام وترسله إلى سجل التدقيق؛ وتبقى قاعدة البيانات هي الحاجز النهائي للصلاحية والسبب والحالة والإصدار. تؤكد الواجهة أن الاعتماد لا ينفذ حركة مخزون أو قيدًا، ولا تحتوي استدعاء تنفيذ أو عكس أو إلغاء أو كتابة مباشرة في الجداول. اجتازت فحص الأنواع وESLint والبناء وجميع الاختبارات وعددها `610` في `60` ملفًا. ما زالت هذه الدفعة محلية: لم تُرفع إلى GitHub ولم تُنشر على Staging، ولم تتغير Staging أو بيئتا الإنتاج.
