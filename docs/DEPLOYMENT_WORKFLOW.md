...
إذا كانت المتغيرات محفوظة بصيغة dotenv، فحمّل الملف بواسطة shell حتى تُزال علامات الاقتباس وفق الصيغة، مثال عام:

```bash
set -a
source /path/to/protected/deploy.env
set +a
```

**ممنوع** استخراج المفتاح بواسطة `sed` أو `cut` ثم تمرير النص الخام إلى البناء؛ قد تبقى علامتا الاقتباس جزءاً من القيمة، فينتج بناء صالح شكلياً لكن يرسل مفتاح API غير صالح ويعيد Supabase الحالة `401 Unauthorized`. يفحص `scripts/deploy-all.sh` الآن المفتاح قبل السحب أو الترحيلات أو البناء، ويرفض القيمة المفقودة أو المقتبسة أو المحتوية على مسافات أو غير المطابقة لصيغة مفتاح Supabase العام. بعد فحص الصيغة يطلب `company_settings` من API المستهدف ويشترط الحالة 200، لأن المفتاح قد يكون صحيح الصيغة لكنه يخص مشروعاً آخر.

مصدر مفاتيح النشر المعتمد على الخادم هو `/etc/accounting-stores-app/deploy.env` خارج Git، داخل مجلد بوضع `0750` وملف بوضع `0640` ومتاح لمجموعة `deploy`. لا تستخدم `.env` من checkout قديم بوصفه مصدراً تلقائياً، ولا تطبع قيم الملف أثناء الفحص.
