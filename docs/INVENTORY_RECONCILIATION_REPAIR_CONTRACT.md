...
أظهرت المراجعة البصرية أن سجل الأحداث كان يعرض `actor_id` مختصرًا تحت مسمى «فاعل». أُعد محليًا تحسين يعرض «نفّذ بواسطة: الاسم — الدور» من `profiles` و`user_roles` بطلبين مجمعين لكل المنفذين، مع احترام RLS وعدم كشف UUID. يُدمج اسم ودور المستخدم الحالي من سياق الجلسة، ويظهر البديل «مستخدم مخوّل» إذا حجبت السياسات بيانات مستخدم آخر. اجتاز التحسين فحص الأنواع وESLint والبناء و`603` اختبارات؛ ولم يُدفع أو يُنشر بعد.
