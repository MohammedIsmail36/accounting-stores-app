...
بُنيت بعدها شاشة تفاصيل للقراءة على المسار `/reports/inventory-reconciliation/repairs/:id`، ويفتحها صف السجل. تعرض الرأس والحالة والإصدار ولقطة التشخيص والبنود والحالة قبل/المقترحة وسجل الأحداث وآثار التنفيذ، مع روابط للسجلات المعروفة. لا تحتوي استدعاءات إنشاء أو تعديل أو اعتماد أو تنفيذ. اجتازت فحص الأنواع وESLint والبناء و`592` اختبارًا، وتنتظر الرفع والنشر على Staging. بعد قبولها تبدأ دفعة مستقلة لإعداد المسودة، مع إبقاء التنفيذ الفعلي محجوبًا حتى المرحلة 2C.
