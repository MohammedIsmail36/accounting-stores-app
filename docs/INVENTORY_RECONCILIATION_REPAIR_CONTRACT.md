...
بُنيت محليًا بعد ذلك دفعة «إرسال المسودة للمراجعة» فقط. يظهر الإجراء للمسودة التي تحتوي بندًا واحدًا على الأقل، ويستدعي `submit_inventory_reconciliation_repair` مع `expected_version` و`request_id` ثابت للمحاولة. بعد نجاح الانتقال إلى `ready_for_review` تختفي إجراءات التعديل والإرسال. لا تستدعي هذه الدفعة إنشاءً أو تعديلًا أو اعتمادًا أو تنفيذًا، ولا تكتب مباشرة في جداول المعالجة أو بيانات الأعمال. اجتازت فحص الأنواع وESLint والبناء وجميع الاختبارات وعددها `606` في `59` ملفًا. ما زالت هذه الدفعة محلية: لم تُرفع إلى GitHub ولم تُنشر على Staging، ولم يتغير الإنتاج.
